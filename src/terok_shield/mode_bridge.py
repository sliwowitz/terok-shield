...
def _ensure_netns(loopback_ports: tuple[int, ...]) -> None:
    """Ensure rootless-netns has the shield nft table with current loopback ports.

    If the table exists but uses different ports, it is replaced.
    ...
    """
    expected = bridge_ruleset(BRIDGE_GATEWAY, BRIDGE_SUBNET, loopback_ports)
    ...
    if "terok_shield" in out:
        if all(f"th dport {p}" in out for p in loopback_ports):
            return
        # Ports changed — delete stale table before reloading.
...
def pre_start(
    ...
    """
    _ensure_netns(config.loopback_ports)
