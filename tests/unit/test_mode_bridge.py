...
class TestPreStart(unittest.TestCase):
    ...
    def _config(self, loopback_ports=()):
        return ShieldConfig(mode=ShieldMode.BRIDGE, loopback_ports=loopback_ports)
    ...
    def test_ensure_netns_reloads_on_port_change(self, mock_nft, _compose, _resolve):
        """Pre-start reloads table when loopback ports have changed."""
        ...
        pre_start(self._config(loopback_ports=(1234,)), "test", ["dev-hardened"])
