...
class TestBridgeRuleset(unittest.TestCase):
    ...
    def test_loopback_ports_present(self) -> None:
        """Loopback ports must appear in ruleset."""
        rs = bridge_ruleset(loopback_ports=(9418, 8080))
        self.assertIn("tcp dport 9418", rs)
        self.assertIn("tcp dport 8080", rs)

    def test_no_loopback_ports(self) -> None:
        """No port rules when loopback_ports is empty."""
        rs = bridge_ruleset(loopback_ports=())
        self.assertNotIn("tcp dport 9418", rs)
    ...
    def test_rejects_invalid_loopback_port(self) -> None:
        ...
        with self.assertRaises(ValueError):
            bridge_ruleset(loopback_ports=(0,))
