...
class TestHookEntrypointStory:
    ...
    def test_full_story_hook_applies_ruleset_and_gateway_baked_in(
        ...
        # 1. Ruleset was pre-generated by pre_start — no dynamic gateway sets
        ruleset_nft = state.ruleset_path(sd)
        assert ruleset_nft.exists(), "ruleset.nft must exist after pre_start"
        ruleset_text = ruleset_nft.read_text()
        assert "set gateway_v4" not in ruleset_text, "dynamic gateway sets must not exist"
        assert "@gateway_v4" not in ruleset_text, "dynamic gateway set refs must not exist"
