...
`(profile.allowed ∪ live.allowed) − deny.list`. Allow-set state the
container learned through dnsmasq survives the transition — a down/up
round trip never forgets IPs the workload already resolved.
